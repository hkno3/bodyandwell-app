...
class WordPressApiService {
  ...
  // 필요한 필드만 요청 (응답 크기 80~90% 감소)
  static const String _fields = 'id,title,excerpt,date,link';
  ...
  Future<List<Article>> fetchLatestArticles({int page = 1, int perPage = 100}) async {
    final cacheKey = 'latest_${page}_$perPage';
    ...
    try {
      final response = await http.get(
        Uri.parse('$apiEndpoint?page=$page&per_page=$perPage&orderby=date&order=desc&_fields=$_fields'),
        headers: {
  ...
  Article _parseWordPressPost(Map<String, dynamic> json) {
    ...
    final category = _categorizeContent(title);
  ...
  String _categorizeContent(String title) {
    return title;
  }
  ...
  Future<List<Article>> fetchArticlesByCategory(String category, {int page = 1, int perPage = 100}) async {
    final cacheKey = 'category_${category}_${page}_$perPage';
    if (_cache.containsKey(cacheKey)) {
      return _cache[cacheKey]!;
    }
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        _cache[cacheKey] = filteredArticles;
  ...
  Future<List<Article>> searchArticles(String query) async {
    try {
      final response = await http.get(
        Uri.parse('$apiEndpoint?search=${Uri.encodeComponent(query)}&per_page=20&_fields=$_fields'),
        headers: {
          'User-Agent': 'BodyAndWell-App/1.0',
          'Accept': 'application/json',
        },
      ...
      if (response.statusCode == 200) {
        ...
        final allArticles = await fetchLatestArticles(page: 1, perPage: 100);
